...
turn_speed = 8    # Higher turn speed for easier turning
...
flip_duration = 120   # Number of frames for flip animation (exactly 2 seconds at 60fps) 
...
def update_physics():
    ...
    # Defender (P2) Constraints - "Guarding the Goal"
    # Defender stays at one end of the field (near their goal)
    if def_pos[0] > 0: # Right Goal Defender (Default for blue if red attacks)
        def_pos[0] = max(FIELD_LENGTH/2 - 120, min(FIELD_LENGTH/2 - CAR_LENGTH/2, def_pos[0]))
    else: # Left Goal Defender
        def_pos[0] = max(-FIELD_LENGTH/2 + CAR_LENGTH/2, min(-FIELD_LENGTH/2 + 120, def_pos[0]))
    # Boundary check for Y as well
...
def showScreen():
    ...
    draw_text(10, 710, "P1: WASD | P2: Arrows | Cam: I/J/K/L | R: Restart", color=(1,1,1))
